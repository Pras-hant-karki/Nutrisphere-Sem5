...
export default function RequestPlanPage() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setPreferredTime("");
      setSpecialRequest("");
      setSuccessMessage("Plan requested sucessfully");
      loadMyRequests();
    } catch (error: any) {
      setMessage({
        type: "error",
        ...
        <div className="w-full max-w-6xl space-y-8">
          ...
          >
            ...
            <div className="space-y-3">
              <p className="text-white text-sm">Diet Type</p>
              <div className="flex items-center gap-6">
                <OptionRadio checked={dietType === "veg"} onChange={() => setDietType("veg")} label="Veg" />
                <OptionRadio checked={dietType === "non-veg"} onChange={() => setDietType("non-veg")} label="Non-Veg" />
              </div>
            </div>
            ...
            <div className="space-y-3">
              <p className="text-white text-sm">Any Medical Condition</p>
              <div className="flex items-center gap-6">
                <OptionRadio checked={medical === "No"} onChange={() => setMedical("No")} label="No" />
                <OptionRadio checked={medical === "Yes"} onChange={() => setMedical("Yes")} label="Yes" />
              </div>
            </div>
